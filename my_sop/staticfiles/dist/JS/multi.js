(function ($) {
    ...
    $.fn.fSelect = function (options) {
        ...
        if (typeof options == 'string') {
            ...
            var settings = $.extend({
                ...
                numDisplayed: 3,//设置页面上可以显示几个选项（超出这个数量将会显示选择了n项）
                overflowText: '选择了{n}项',//超出这个数量将会显示"选择了n项"
                searchText: '搜索',//搜索框中占位符显示什么字
                showSearch: true,//是否显示搜索框
                multipleSearch: false,//是否支持多项搜索
            }, options);
        }

        if (options !== undefined && options.method !== undefined && options.method == 'setValues') {
            settings = options.method;
            valueArray = options.valueArray;
        }
        ...
        fSelect.prototype = {
            ...
            reload: function () {
                ...
                if (this.settings.showSearch) {
                    let multipleSearch = this.settings.multipleSearch ? 1 : 0;
                    var search = '<div class="fs-search"><input type="search" placeholder="' + this.settings.searchText + '" data-multiple-search="' + multipleSearch + '"/></div>';
            ...
            reloadFilterHidden: function () {
                var choices = this.buildOptions(this.$select, true);
                this.$wrap.find('.fs-options').empty();
                this.$wrap.find('.fs-options').append(choices);
            },
            ...
            buildOptions: function ($element, hiddenFilter = false) {
                var $this = this;

                var choices = '';
                var selectorFilter = hiddenFilter ? "[data-hidden!='true']" : "";
                $element.children(selectorFilter).each(function (i, el) {
                    var $el = $(el);
                    //支持 optiongroup
        ...
        return this.each(function () {
            ...
            if (typeof settings == 'string' && settings != 'setValues') {
                data[settings]();
            } else if (typeof settings == 'string' && settings == 'setValues') {
                data[settings](valueArray);
            }
        });
    }
    /**
    ...
    $(document).on('keyup', '.fs-search input', function (e) {
        ...
        if (this.dataset.multipleSearch == 1) {
            keywords = keywords.replaceAll(' ', '');
            keywords_array = keywords.split(',');
            keywords_array = keywords_array.filter(word => word != '');
