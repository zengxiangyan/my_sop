...
class VideoBase(object):
    ...
    def process_data(self, process, task, start=0, end=0, batch='', where='', test=-1):
        ...
        basic_condition = f"clean_id={self.clean_id} and category={self.category}"
        aweme_sql = f"""
            select id, aweme_id from {self.run_table}
            where {basic_condition} and {where_condition} {order_condition} for update;
        """
        ...
        def callback(id_list):
            ...
            if task in (1, 13) or self.data_type in (2, 3, 4, 6):
                sql = data_sql[task].format(version=self.video_version, category=self.category, ids=ids, col=clean2chatcol[self.clean], tpp=task)
                if test > 0:
                    print(sql)
                data = self.dy2.query_all(sql)
            else:
                sql = data_sql[task].format(ids=ids, tpp=task)
                if test > 0:
                    print(sql)
                data = self.db_sop.query_all(sql, print_sql=False)
            ...
            if test > 0:
                ...
                if len(clean_result) > 0:
                    ...
                    columns = ['aweme_id'] + self.columns + clean2columntail[self.clean]
        ...
        if start == 0:
            start = self.quick_start(" and ".join([basic_condition, where if where else "1", batch_condition, task_condition]))
    ...
    def process_data_on_multitasks(self, process, multi_tasks, start=0, end=0, batch='', where='', test=-1,
        ...
        aweme_sql = f"""
            select id, aweme_id from {self.run_table}
            where {basic_condition} and {where_condition} {order_condition};
        """
        ...
        def callback(id_list):
            ...
            for task in multi_tasks:
                if task in (1, 13) or self.data_type in (2, 3, 4, 6):
                    ...
                    data = self.db_sop.query_all(sql, print_sql=False)
                # ee = time.time()
                # print(f'{task} done takes {ee - ss}s\n')
    ...
    def apply_clean(self, batch='', start=0, end=0, where='', remove_zero=True, tmp=False):
        ...
        traverse_sql = f"select id, aweme_id from {self.run_table} where clean_id={self.clean_id} and category={self.category} and {where_condition} and id>%d order by id limit %d;"
        ...
        def dy_callback_2(id_list):
            ...
            insert = []
            update = []
            if self.data_type in (5, 7):  # 全品类项目特殊挂车
            ...
            if len(delete) > 0:
                ...
                del_flag = check_delete(self.dy2, result_table)
                while not del_flag:
                    time.sleep(1)
                    del_flag = check_delete(self.dy2, result_table)
                self.dy2.execute(delete_sql)
        ...
        utils.easy_traverse(self.dy2, traverse_sql, dy_callback_2, 0, 1000, print_sql=True)

    @used_time
    def apply_manual(self, batch='', start=0, end=0, where='', tmp=False, **kwargs):
        """应用人工值"""
        result_table = f'{self.result_table}_tmp' if tmp else self.result_table
        # manual_sql = f"""
        ...
        awm_condition = f"aweme_id in (select aweme_id from {self.run_table} where clean_id={self.clean_id} and {where_condition})" if batch else "1"
        manual_sql = f"""
            select aweme_id from {self.manual_table}
            where category={self.category} and {awm_condition}
                  and aweme_id>%d order by aweme_id limit %d
        """
        def dy_callback(data):
            ...
            '''
            datainfo = self.dy2.query_all(sql)
    ...
    def quick_start(self, condition):
        """从最小符合的消息队列开始执行"""
        sql = f"select min(id) from {self.run_table} where {condition};"
        min_id = self.dy2.query_all(sql)
        return min_id[0][0] - 1 if len(min_id) > 0 and isinstance(min_id[0][0], int) else 0

    @used_time
    def move_to_mention(self, batch='', start=0, end=0):
        move_to_mention(self.dy2, self.category, self.clean, prefix=self.video_version, batch=batch, start=start, end=end, clean_id=self.clean_id)
