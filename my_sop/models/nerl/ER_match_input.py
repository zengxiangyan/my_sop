...
def entity_embedding(config):
    """
    得到实体名字的嵌入

    :return:
    """
    entity_id = get_entity_id(config)
    print(len(entity_id))
    id_embedding = pd.read_pickle(join(app.output_path(config.data.data_dir), 'id_embedding.pkl'))
    entity_index = {}
    ...
    pd.to_pickle(entity_index, join(app.output_path(config.data.data_dir), 'ER_entity_index.pkl'))
    np.save(join(app.output_path(config.data.data_dir), 'ER_entity_embedding.npy'), embedding_matrix)
...
def get_input_match(config, max_len=52, mode="train"):
    """
    构建模型的训练集输入
    :param mode:
    :param config:
    ...
    output_file = join(app.output_path(config.data.data_dir), f'input_{mode}_match.pkl')
    bert_dir = config.data.bert_dir

    token_dict = get_token_dict(bert_dir)
    tokenizer = Tokenizer(token_dict)
    split_entity = pd.read_pickle(join(app.output_path(config.data.data_dir), 'split_entity.pkl'))

    trie_obj = get_Trie(config)
    entity_index = pd.read_pickle(join(app.output_path(config.data.data_dir), 'ER_entity_index.pkl'))
...
if __name__ == '__main__':
    # entity_embedding()
    # get_input_bert('data/ER_input_train_match.pkl')
    # get_input_bert_test('original_data/develop.json', 'data/ER_input_test_match.pkl')
