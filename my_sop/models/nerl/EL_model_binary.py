...
from models.nerl.keras_layers import *
...
def link_model(bert_dir):
    ...
    bert_model = load_trained_model_from_checkpoint(config_path, checkpoint_path, trainable=True, seq_len=256)
    x = bert_model.output
...
def train_nel(config):
    model_dir = app.output_path(config.train.model_dir)
    dataset = get_input(app.output_path(join(config.data.data_dir, 'input_train_nel.pkl')))
    train = dataset
    ...
    for i, (tra_index, val_index) in enumerate(kfold.split(train[0])):
        ...
        filepath_loss = join(model_dir, "EL_model_bert_loss.h5_" + str(i))
        filepath_f1 = join(model_dir, "EL_model_bert_f1.h5_" + str(i))

        evaluate = Evaluate((input_val[:-1], input_val[-1]), filepath_f1)
        checkpoint = ModelCheckpoint(filepath_loss, monitor='val_loss', verbose=2, save_weights_only=True,
        ...
        model = link_model(config.data.bert_dir)
        print(model.summary())
        print(i)
        # #
        logging.debug(str(i) + '################')
        model = model.fit(input_train[:-1], input_train[-1], batch_size=batch_size, epochs=num_epochs,
...
def predict_loss(config, input_file='input_develop_nel.json', out_file='result_develop_nel.json', model_index=0):
    model_dir = app.output_path(config.train.model_dir)
    input_file = join(app.output_path(config.data.data_dir), input_file)
    out_file = join(app.output_path(config.data.result_dir), f'{model_index}_loss_{out_file}')
        ...
        with open(input_file, 'r') as f:
            for j, line in enumerate(f):
                ...
                for men, re_men in zip(mention_data, re_men_data):
                    if len(men['link_id']) > 0:
                        pred = model.predict(get_input(men['link_data'], mode='test'))
                        re_men['link_pred'] = list(
                            np.sum([re_men['link_pred'], list(np.squeeze(pred, axis=-1))], axis=0))
                    else:
                        re_men['link_pred'] = []
...
def predict_f1(config, input_file='input_develop_nel.json', out_file='result_develop_nel.json', model_index=0):
    model_dir = app.output_path(config.train.model_dir)
    input_file = join(app.output_path(config.data.data_dir), input_file)
    out_file = join(app.output_path(config.data.result_dir), f'{model_index}_f1_{out_file}')
    ...
    for i in range(5):
        if not i == model_index:
            continue
        print(i)
        filepath_loss = join(model_dir, "EL_model_bert_f1.h5_" + str(i))
        model = link_model(config.data.bert_dir)
        model.load_weights(filepath_loss)
        with open(input_file, 'r') as f:
            for j, line in enumerate(f):
...
if __name__ == '__main__':
    pass
    # train()
    # # predict('data/test_link_binary_bert.json', 'result/test_link_result_bert2.json')
    # # predict('data/dev_link_binary_bert.json', 'result/dev_link_result_bert_final.json')
    #
    #
