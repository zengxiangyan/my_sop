...
def get_Trie(config, min_len=2):
    """
    构建实体字典树，长度小于2的不插入
    :param min_len: 实体长度
    :return:
    """
    kb_en = pd.read_pickle(app.output_path(join(config.data.data_dir, 'entity_id.pkl')))
...
def get_split_entity(config):
    ...
    pd.to_pickle(split_dict, app.output_path(join(config.data.data_dir, 'split_entity.pkl')))
