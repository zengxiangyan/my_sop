...
import pandas as pd
import numpy as np
from models.nerl.data_utils import *
from models.nerl.trie import *
import random
from keras_bert import Tokenizer
...
def get_link_entity(mention, id, entity_id, id_entity):
    ...
    if mention in entity_id:
        ...
        link_entitys += list(entity_id[id_entity[str(id)]])
    else:
        link_entitys += list(entity_id[id_entity[str(id)]])
    link_entitys = set(link_entitys)
    link_entitys.remove(str(id))
...
def get_input(config, mode='train'):
    """
    消歧输入的构建

    :return:
    """
    id_type = pd.read_pickle(app.output_path(join(config.data.data_dir, 'id_type.pkl')))
    type_index = pd.read_pickle(app.output_path(join(config.data.data_dir, 'type_index.pkl')))
    entity_id = pd.read_pickle(app.output_path(join(config.data.data_dir, 'entity_id.pkl')))
    id_entity = pd.read_pickle(app.output_path(join(config.data.data_dir, 'id_entity.pkl')))
    id_text = pd.read_pickle(app.output_path(join(config.data.data_dir, 'id_text.pkl')))
    inputs = {'ids': [], 'seg': [], 'begin': [], 'end': [], 'en_type': [], 'labels': []}
    token_dict = get_token_dict(config.data.bert_dir)
    tokenizer = Tokenizer(token_dict)
    file_len = 0
    trie_obj = get_Trie(config)
    ...
    if mode == 'train':
            ...
            for line in f:
                ...
                for men in mention_data:
                    ...
                    if kb_id != -1:
                        ...
                        for id in link_id:
                            kb_text = id_text[str(id)]
                            kb_type = type_index[id_type[str(id)][0]]
        ...
        for k in inputs:
            ...
            print(inputs[k][1])
        pd.to_pickle(inputs, app.output_path(join(config.data.data_dir, 'input_train_nel.pkl')))
    else:
        file_index = 0
        out_file = open(app.output_path(join(config.data.data_dir, f'input_{mode}_nel.json')), 'w')
        with open(app.output_path(join(config.data.raw_dir, f'{mode}.json'))) as f:
            for line in f:
                if file_index % 100 == 0:
                    print(file_index)
                file_index += 1
...
# def get_test_input(config, input_file, out_file):
