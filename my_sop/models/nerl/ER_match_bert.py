...
def link_model(config):
    ...
    embedding_matrix_entity = np.load(join(app.output_path(config.data.data_dir), 'ER_entity_embedding.npy'))
    bert_model = load_trained_model_from_checkpoint(config_path, checkpoint_path, trainable=True, seq_len=52)
    entity_embedding = Embedding(input_dim=312452, output_dim=768, weights=[embedding_matrix_entity], trainable=True,
...
def get_input(input_file):
    ...
    inputs = [data['ids'], data['seg'], data['entity_id'], data['begin'], data['end'],
...
def train_match(config):
    """
    模型训练，将训练集分为9份，9折交叉验证，分别按照loss和f1保存模型。

    :return:
    """
    train_file = join(app.output_path(config.data.data_dir), 'input_train_match.pkl')
    model_dir = app.output_path(config.train.model_dir)

    dataset = get_input(train_file)
    train = dataset
    ...
    for i, (tra_index, val_index) in enumerate(kfold.split(train[0])):
        ...
        filepath_loss = join(model_dir, "ER_match_bert_loss.h5_" + str(i))
        filepath_f1 = join(model_dir, "ER_match_bert_f1.h5_" + str(i))
        evaluate = Evaluate((input_val[:-1], input_val[-1]), filepath_f1)
        checkpoint = ModelCheckpoint(filepath_loss, monitor='val_loss', verbose=2, save_best_only=True,
...
def predict_match(config, input_file='input_develop_match.pkl', out_file='result_develop_match.npy'):
    """
    ...
    for i in range(config.train.k_fold):
        ...
        K.clear_session()
        model = link_model(config)
        filepath_loss = join(model_dir, "ER_match_bert_f1.h5_" + str(i))
