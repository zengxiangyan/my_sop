...
from models.nerl.evaluate import mention_evaluate
from models.nerl.trie import entity_link_prob
...
def get_NER_result(config, input_file='develop.json', ner_pred_file='result_develop_ner.npy', match_file='input_develop_match.pkl',
                   match_pred='result_develop_match.npy', out_file='result_ER.json'):
    """
    ...
    :return:
    """
    input_file = join(app.output_path(config.data.result_dir), input_file)
    ner_pred_file = join(app.output_path(config.data.result_dir), ner_pred_file)
    match_file = join(app.output_path(config.data.result_dir), match_file)
    match_pred = join(app.output_path(config.data.result_dir), match_pred)
    ...
    link_prob = entity_link_prob(config)
...
if __name__ == '__main__':
    # del_NIL()
    # get_result('data/dev.json','data/pred_dev_ner.npy','data/input_dev_match_bert.pkl',
    #            'data/pred_dev_match.npy','result/dev_ner_result.json')
