...
class main(calculate_sp.main):
    ...
    def start(self, batch_now, read_data, other_info=None):
        ...
        for row in self.read_data:
            ...
            if self.result_dict[item_id]['mp7'] != '':
                self.result_dict[item_id]['sp7'] = self.process_sp7(self.result_dict[item_id]['mp7'])

            if self.result_dict[item_id]['sp5'] == '其它' and f_map['shop_type'] in (22, 24):
                self.result_dict[item_id]['sp5'] = '进口'

            self.batch_now.print_log('id =', item_id, self.result_dict[item_id])

        return self.result_dict

    def process_sp7(self, ori_mp7):
        return ori_mp7.replace('&', self.split_word)