...
class main(calculate_sp.main):
        ...
        for row in self.read_data:
            ...
            if source == 11:
                ...
                if self.hash_id_info.get(hash_id) != None:
                    ...
                    sp[7] = str(hash_info['ner_bid'])
                    sp[8] = hash_info['ner_brand']
                if alias_all_bid == 0 or alias_all_bid == null:
                    flag = True
                    alias_all_bid = hash_info['ner_bid']
                    sp[6] = str(hash_info['ner_bid'])
    ...
    def init_read_require(self):
        ...
        # 读取sp4配置
        self.uuid_info = {}
        sql = "select uuid2, ner_bid, ner_brand from sop_c.entity_prod_91848_C_tiktok"
        for row in self.batch_now.db_chsop.query_all(sql):
            self.uuid_info[str(row[0])] = {
                'ner_bid': row[1],
                'ner_brand': row[2],
            }