import sys
import time
import datetime
from os.path import abspath, join, dirname
sys.path.insert(0, join(abspath(dirname(__file__)), '../../'))
# sys.stdout = io.TextIOWrapper(sys.stdout.buffer,encoding='utf8')
import models.plugins.batch as Batch
import application as app


class main(Batch.main):

    def brush(self, smonth, emonth, logId=-1):
        # sql = 'SELECT source, tb_item_id, real_p1 FROM {}'.format(self.cleaner.get_tbl())
        # ret = self.cleaner.db26.query_all(sql)
        # mpp = {'{}-{}-{}'.format(v[0], v[1], v[2]): True for v in ret}

        ####################1
        clean_flag = self.cleaner.last_clean_flag() + 1
        uuids1 = []
        ret1, sales_by_uuid = self.cleaner.process_top_by_cid(smonth, emonth, limit=1500, others_ratio=0.05)
        for uuid2, source, tb_item_id, p1,cid,alias_all_bid in ret1:
            # if '{}-{}-{}'.format(source, tb_item_id, p1) in mpp:
            #     continue
            if self.skip_brush(source, tb_item_id, p1):
                continue
            uuids1.append(str(uuid2))
        self.cleaner.add_brush(uuids1, clean_flag, 1, sales_by_uuid=sales_by_uuid)
        ####################2
        uuids2 = []
        ret2, sales_by_uuid2 = self.cleaner.process_top_by_cid(smonth, emonth, limit=1000, random=True, others_ratio=0.05)
        for uuid2, source, tb_item_id, p1, cid, alias_all_bid in ret2:
            # if '{}-{}-{}'.format(source, tb_item_id, p1) in mpp:
            #     continue
            if self.skip_brush(source, tb_item_id, p1):
                continue
            uuids2.append(str(uuid2))

        self.cleaner.add_brush(uuids2, clean_flag, 2, sales_by_uuid=sales_by_uuid2)
        # print(len(uuids1), len(uuids2))

        return True