...
class main(Batch.main):
    ...
    def calc_splitrate(self, item, data, custom_price={}):
        ...
        follow_pids = [6,8,10,71,72,73,74,75,76,77,78,79,80,81,82,83,84,85]
        ...
        if 'sku_price2' not in self.cleaner.cache:
            ...
            sql = '''
                ...
                WHERE b_similarity = 0 AND b_pid IN ({}) ORDER BY transform(s,[1,2,8],[999,998,-1],s) DESC
                LIMIT 1 BY k
    ...
    def hotfix_new(self, tbl, dba, prefix):
        ...
        # 找出只有71和2混合的拆套包
        sql = '''
            INSERT INTO {t}_join2
            select uuid2, clean_sku_id, 64500 as mprice
            from {t}
            where clean_sku_id = 71 and clean_split > 0 AND clean_brush_id > 0 and uuid2 > 0 and clean_num != 0
        '''.format(t=tbl)
        dba.execute(sql)
