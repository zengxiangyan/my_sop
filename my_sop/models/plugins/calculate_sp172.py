...
class main(calculate_sp.main):
    ...
    def start(self, batch_now, read_data, other_info=None):
        ...
        for row in self.read_data:
            ...
            for a,b,c in [['sp18', 'sp20', 'sp42'], ['sp51', 'sp44', 'sp45']]:
                if self.result_dict[item_id][a] != '' and self.result_dict[item_id][b] != '':
                    num = round(float(self.result_dict[item_id][a]) * float(self.result_dict[item_id][b]), 1)
                    if num % 1 == 0:
                        num = int(num)
                    self.result_dict[item_id][c] = str(num)
                elif self.result_dict[item_id][a] == '' and self.result_dict[item_id][b] != '' and float(self.result_dict[item_id][b]) != 0 and self.result_dict[item_id][c] != '':
                    num = round(float(self.result_dict[item_id][c]) / float(self.result_dict[item_id][b]), 1)
                    if num % 1 == 0:
                        num = int(num)
                    else:
                        num = self.result_dict[item_id][c]
                        self.result_dict[item_id][b] = '1'
                    self.result_dict[item_id][a] = str(num)
    ...
    def process_row(self, item_id, source, cid, all_bid, alias_all_bid, prop_all, trade_prop_all, f_map):
        ...
        for pos_id in self.batch_now.pos_id_list:
            ...
            if self.batch_now.pos[pos_id]['type'] == 900: # 子品类
                ...
                if sub_batch_id > 0 or pos_id in (18, 20, 41, 42, 44, 45, 51):
                    can_pure_num = []
                    if self.batch_now.pos[pos_id]['p_key']:
                        seq = self.batch_now.pos[pos_id]['p_key'].get(source+str(cid), 'product,name').split(',')
                        for combined_keys in seq:
