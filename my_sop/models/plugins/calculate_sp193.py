...
class main(calculate_sp.main):
    ...
    def start(self, batch_now, read_data, other_info=None):
        ...
        for row in self.read_data:
            ...
            if self.hash_id_info.get(hash_id) != None:
                ...
                if hash_info['classify_category'] != '其它':
                    sp[7] = '即食品'
                else:
                    sp[7] = '其它'
                if '-' in hash_info['classify_category']:
                    sp[8], sp[9] = hash_info['classify_category'].split('-')
                sp[10] = hash_info['ner_bid']
                sp[11] = hash_info['ner_brand']
    ...
    def init_read_require(self):
        ...
        # 读取sp4配置
        self.hash_id_info = {}
        sql = "select hash_id, ner_bid, ner_brand, classify_category from sop_c.entity_prod_91485_C_tiktok"
