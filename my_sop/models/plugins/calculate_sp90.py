...
class main(calculate_sp.main):
    ...
    def start(self, batch_now, read_data, other_info=None):
        ...
        for row in self.read_data:
            ...
            mp[9], sp[9] = self.formula(sp, detail)
    ...
    def formula(self, sp, detail):
        mp9, sp9 = ('', '')
        if detail.get(8):
            # print('Pos 8 detail:', detail[8])
            ans = []
            for lis in detail[8][0][0][2]:
                after = [self.batch_now.unit_conversion(self.batch_now.to_digit(lis[1], ''), ''), self.batch_now.unit_conversion(self.batch_now.to_digit(lis[3], ''), lis[4], self.batch_now.pos[8]['default_quantifier'], self.batch_now.pos[8]['unit_conversion']), self.batch_now.unit_conversion(self.batch_now.to_digit(lis[6], ''), ''), self.batch_now.unit_conversion(self.batch_now.to_digit(lis[8], ''), '')]
                # print(after)
