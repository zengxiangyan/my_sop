...
class main(calculate_sp.main):
    ...
    def start(self, batch_now, read_data, other_info=None):
        ...
        for row in self.read_data:
            ...
            self.result_dict[uuid2]['all_bid_sp'] = self.process_dy(f_map['snum'], alias_all_bid, f_map, self.result_dict[uuid2]['all_bid_sp'])
            
            item_id = f_map['item_id']
            info = self.update_bid.get((item_id, '|||'.join(trade_prop_all.values())))
            if info != None:
                month_str = f_map['month'][:4] + f_map['month'][5:7]
                if month_str >= info['start_month'] and month_str <= info['end_month'] and sp[3] == info['sp3'] and f_map['source'] == info['platform']:
    ...
    def init_read_require(self):
        ...
        sql = 'select start_month, end_month, sp3, platform, item_id, trade_value, all_bid_sp from clean_93_update_bid'
        for row in self.batch_now.db_cleaner.query_all(sql):
            start_month, end_month, sp3, platform, item_id, trade_value, all_bid_sp = list(row)
        ...
        sql = 'select start_month, end_month, platform, item_id, trade_value, sub_batch_id, sp3 from clean_93_update_sp3'
        for row in self.batch_now.db_cleaner.query_all(sql):
            start_month, end_month, platform, item_id, trade_value, sub_batch_id, sp3 = list(row)
