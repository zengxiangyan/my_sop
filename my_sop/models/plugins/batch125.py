...
class main(Batch.main):
    ...
    def brush(self, smonth, emonth):
        ...
        uuids1 = []
        ret1, sales_by_uuid = self.cleaner.process_top_new_default(smonth, emonth, others_ratio=0.05, limit=2000)
        for uuid2, source, tb_item_id, p1, in ret1:
            if '{}-{}-{}'.format(source, tb_item_id, p1) in mpp:
                continue
            uuids1.append(uuid2)
        uuids2 = []
        ret2 = self.cleaner.process_rand_new_default(smonth, emonth, others_ratio=0.05, limit=1000)
