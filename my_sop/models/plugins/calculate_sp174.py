...
class main(calculate_sp.main):
    ...
    def start(self, batch_now, read_data, other_info=None):
        ...
        for row in self.read_data:
            ...
            if cid not in self.cid_list and f_map['pkey'] >= '2021-12-01':
                self.cid_list.append(cid)
                self.get_cid_require(cid)

            if f_map['pkey'] >= '2021-12-01':
                prop_name_list = []
                prop_value_list = []
                ...
                if f_map['snum'] == 11 and self.hash_id_info.get(hash_id) != None:
                    ...
                    sp[15] = info['ner_bid']
                    sp[16] = info['ner_brand']
                    sp[18] = info['classify_category']
    ...
    def get_cid_require(self, cid):
        sql = "select hash_id, ner_bid, ner_brand, classify_category from sop_c.entity_prod_91363_C_tiktok where cid = " + str(cid)
    ...
    #     # 读取配置
    #     self.hash_id_info = {}
    #     sql = "select hash_id, ner_bid, ner_brand, classify_category from sop_c.entity_prod_91363_C_tiktok where uuid2 = '7a85aef9-8606-4318-96c5-f16b09d858ad'"
    #     for row in self.batch_now.db_chsop.query_all(sql):
    #         self.hash_id_info[str(row[0])] = {
    #             'ner_bid': row[1],
    #             'ner_brand': row[2],
    ...
    def process_row(self, item_id, source, cid, all_bid, alias_all_bid, prop_all, trade_prop_all, f_map):
        ...
        for pos_id in self.batch_now.pos_id_list:
            ...
            if self.batch_now.pos[pos_id]['type'] == 900: # 子品类
                ...
                if sub_batch_id > 0 or pos_id == 23:
                    can_pure_num = []
                    if self.batch_now.pos[pos_id]['p_key']:
                        seq = self.get_pkey(pos_id, source, cid, 'product,name', prop_all.keys())
                        for combined_keys in seq:
