...
class main(calculate_sp.main):
    ...
    def init_read_require(self):
        sql = '''
            select rule_group, rule_order, ori_cate, l1_no,  l1_en, l2_no,  l2_en, l3_no,  l3_en, number_order, cate_1, cate_2, cate_3, cate_4, cate_result, rule_judg, continue_pro, rule_type, value, NULLIF(other1, ''), NULLIF(other2, ''), NULLIF(other3, ''), NULLIF(other4, ''), NULLIF(other5, ''), NULLIF(other6, ''), NULLIF(other7, ''), NULLIF(other8, ''), NULLIF(other9, ''), NULLIF(other10, ''), NULLIF(other11, ''), NULLIF(other12, ''), NULLIF(other13, ''), NULLIF(other14, ''), NULLIF(other15, '')
            from cleaner.clean_259_category
        '''
        data = self.batch_now.db_cleaner.query_all(sql)
        df = pd.DataFrame(list(data), columns=['规则组', '规则顺序', '原分类', 'L1-NO', 'L1-EN', 'L2-NO', 'L2-EN', 'L3-NO', 'L3-EN', '出数排序', '分类1', '分类2', '分类3', '分类4', '分类结果', '规则判断', '继续处理', '规则类型', 'value', 'Unnamed: 19', 'Unnamed: 20', 'Unnamed: 21', 'Unnamed: 22', 'Unnamed: 23', 'Unnamed: 24', 'Unnamed: 25', 'Unnamed: 26', 'Unnamed: 27', 'Unnamed: 28', 'Unnamed: 29', 'Unnamed: 30', 'Unnamed: 31', 'Unnamed: 32', 'Unnamed: 33'])
        ...
        self.category_classifier.load_rules_from_db(df)
    ...
    def process_row(self, item_id, source, cid, all_bid, alias_all_bid, prop_all, trade_prop_all, f_map):
        ...
        for pos_id in self.batch_now.pos_id_list:
            ...
            if self.batch_now.pos[pos_id]['type'] == 900: # 子品类
                ...
                # import pdb;pdb.set_trace()
                # 不知道c4含义，暂时用 分类 顶上
                data = [{'name': prop_all['trade_prop_all'], 'c4': prop_all.get('分类')},
                        {'name': prop_all['name'], 'c4': prop_all.get('分类')}]

                for row in data:
                    c6 = self.category_classifier.test(row)
                    if c6 != 'TBD':
                        mp[pos_id], sp[pos_id] = row['name'], c6
                        break
