...
class main(Batch.main):
    ...
    def check_shop(self, eid, tbl,source,s_date,e_date):
        dba = self.cleaner.get_db('chsop')
        where = """ date<'{e_date}' """.format(e_date=e_date)
        if source:
            where += 'AND source in ({})'.format(souce)
        sql = '''
        ...
        '''.format(t=tbl,s_date=s_date,where=where)
        dba.execute(sql)
    ...
    def get_fss_shop_info(self, type,eid, tbl,source,alias_bid,date):
        dba = self.cleaner.get_db('chsop')
        bids = self.get_top_bids(eid)
        cols = ['平台','sid','店铺名','url']
        where = """
                tbl = '{t}' AND toYYYYMM(date) = {date}
                """.format(date=date,t=tbl)
        fss = """
                AND (
                        (`source`, sid) IN (
                            SELECT toUInt8(`source`), sid FROM sop_e.shop_91783_type WHERE type_meizhuang IN (1,2) AND sid > 0
        ...
        if alias_bid:
            where += """
                ...
                """.format(alias_bid.replace('，',','),t=tbl)
        
        sql = """
                SELECT sid, name, dictGet('all_shop', 'title', tuple(`source`,sid)) AS new_name FROM artificial.entity_etbl_shopinfo
                WHERE {where}
            """.format(where=where)
        sid_list = dba.query_all(sql)
        sid_list = ['0'] if len(sid_list)==0 else [str(sid[0]) for sid in sid_list]
        ...
        sql = '''
            ...
            LEFT JOIN BBB
            ON AAA.source = BBB.source and AAA.sid = BBB.sid
        '''.format(sid_str=','.join(sid_list),where=where)

        if type == "旗舰改名":
            cols = ['平台','sid','旧店铺名','新店铺名','url']
            sql = '''
            WITH BBB AS (select * from artificial.all_shop WHERE sid in ({sid_str}))
