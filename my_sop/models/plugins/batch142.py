...
class main(Batch.main):
    ...
    def brush(self, smonth, emonth):
        ...
        uuids1 = []
        ret1, sales_by_uuid = self.cleaner.process_top_by_cid(smonth, emonth, limit=2000, others_ratio=0.05, sp=9)
        for uuid2, source, tb_item_id, p1, cid in ret1:
            if self.skip_brush(source, tb_item_id, p1):
                continue
            uuids1.append(uuid2)
        uuids2 = []
        ret2,sales_by_uuid = self.cleaner.process_top_by_cid(smonth, emonth, limit=2000, random=True, others_ratio=0.05, sp=9)
