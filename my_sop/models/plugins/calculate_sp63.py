    ...
    def process_row(self, item_id, source, cid, all_bid, alias_all_bid, prop_all, trade_prop_all, f_map):
        ...
        for pos_id in self.batch_now.pos_id_list:
            ...
            if self.batch_now.pos[pos_id]['type'] == 900: # 子品类
                mp[pos_id], sp[pos_id] = (0, self.batch_now.default_other)
                if f_map.get('sid') in [170685533, 181871121]:
                    mp[pos_id] = 1649
                    self.batch_now.print_log('sid: ', f_map.get('sid'))
                elif f_map.get('tb_item_id') in ['596580221351', '604930324491', '604930144607', '582730210355', '604435038302']:
                    mp[pos_id] = 1650
                    self.batch_now.print_log('tb_item_id: ', f_map.get('tb_item_id'))
                elif alias_all_bid == 22005:
                    mp[pos_id] = 1651
                    self.batch_now.print_log('alias_all_bid: ', alias_all_bid)
                sub_batch_id = mp[pos_id]
                if sub_batch_id > 0:
                    sp[pos_id] = self.batch_now.sub_batch[sub_batch_id]['name']
                clean_type = -1 if sub_batch_id == 0 else self.batch_now.sub_batch[sub_batch_id]['clean_type']    # 不属于本清洗大类
