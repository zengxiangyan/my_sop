...
class DAO:
    ...
    def direct_connect(self):
        ...
        self.con = MySQLdb.connect(host=self.host,port=self.port,user=self.user,passwd=self.passwd,db=self.db,charset=self.charset)
        return self.con
    ...
    def get_engine(self):
        ...
        self.engine = create_engine('mysql+mysqldb://{0}:{1}@{2}:{3}/{4}?charset={5}'.format(self.user, self.passwd, self.host, self.port, self.db, self.charset))
        return self.engine       
    ...
    def query_all(self, sql, param_tuple=None, as_dict=False, with_cols=False,print_sql=True):
        #self.print(sql, param_tuple)
        try_count = 0
        while try_count < self.try_count:
            try:
    ...
    def execute(self, sql, param_tuple=None):
        ...
        while try_count < self.try_count:
            ...
            # except (_mysql_exceptions.OperationalError, _mysql_exceptions.InterfaceError, pymysql.err.OperationalError, pymysql.err.InterfaceError) as e:
                ...
                try_count += 1
                if (e.args[0]!=2006 and e.args[0]!=2013 and e.args[0]!=0 and e.args!=2003) or try_count >= self.try_count:
