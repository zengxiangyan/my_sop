...
def create_instances_from_document_sentence_pairs(
        all_documents, document_index, tokenizer, max_seq_length, short_seq_prob,
        masked_lm_prob, max_predictions_per_seq, vocab_words, rng):
    document = all_documents[document_index]
    max_sequence_length_allowed = max_seq_length - 3

    instances = []
    for question, answer, label in document:
        # 添加random还是答案
        is_random_next = False if label else True
        question_tokens = get_new_segment(tokenizer.tokenize(question[:int(max_sequence_length_allowed * 0.6)]))
        answer_tokens = get_new_segment(tokenizer.tokenize(answer[:int(max_sequence_length_allowed * 0.4)]))
    ...
    for input_file in input_files:
        with tf.gfile.GFile(input_file, "r") as reader:
            while True:
                ...
                all_documents[-1].append((question, answer, label))
